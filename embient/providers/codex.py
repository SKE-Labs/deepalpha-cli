...
def refresh_tokens(refresh_token: str) -> dict:
    ...
    with httpx.Client() as client:
        ...
        if resp.status_code in (401, 403):
            raise RuntimeError("Codex token refresh failed. Re-authenticate with: embient auth codex")
        resp.raise_for_status()
        return resp.json()  # type: ignore[no-any-return]
...
class CodexTokenManager:
    ...
    def _refresh(self) -> str:
        ...
        if not self._refresh_token:
            creds = self._store.load()
            if not creds:
                raise RuntimeError("No Codex credentials stored. Run: embient auth codex")
            self._refresh_token = creds["refresh_token"]
...
def codex_login_interactive() -> None:
    ...
    console.print()
    console.print("Use with:  [cyan]embient --provider codex[/cyan]")
    console.print("       or: [cyan]embient --model codex/gpt-5.3-codex[/cyan]")
    console.print()
