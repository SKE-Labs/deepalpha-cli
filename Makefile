...
lint_diff format_diff: PYTHON_FILES=$(shell git diff --relative=deepalpha-cli --name-only --diff-filter=d master | grep -E '\.py$$|\.ipynb$$')
lint_package: PYTHON_FILES=deepalpha
...
check_imports: $(shell find deepalpha -name '*.py')
