...
class TestGetSystemPrompt:
    ...
    def test_local_mode(self, mock_load: object) -> None:
        ...
        assert body_call[1]["agent_dir_path"] == "~/.embient/test-agent"
        # Result combines both
        assert "\n\n" in result

    @patch("embient.agent.get_default_working_dir", return_value="/workspace")
    @patch("embient.agent.load_prompt")
    ...
    def test_agent_dir_path_in_body(self, mock_load: object) -> None:
        ...
        assert body_call[1]["agent_dir_path"] == "~/.embient/my-agent"
