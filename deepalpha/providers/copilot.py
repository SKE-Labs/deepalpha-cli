    ...
    with httpx.Client() as client:
        ...
        if resp.status_code in (401, 403):
            raise RuntimeError(
                ...
                "Re-authenticate with: deepalpha auth copilot"
            )
        resp.raise_for_status()
        data = resp.json()
...
class CopilotTokenManager:
    ...
    def _refresh(self) -> tuple[str, str]:
        ...
        if not github_token:
            raise RuntimeError("No GitHub token stored. Run: deepalpha auth copilot")
...
def copilot_login_interactive() -> None:
    ...
    console.print()
    console.print("Use with:  [cyan]deepalpha --provider copilot[/cyan]")
    console.print("       or: [cyan]deepalpha --model copilot/claude-sonnet-4-5-20250929[/cyan]")
    console.print()
