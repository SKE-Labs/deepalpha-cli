...
def _refresh_tokens(refresh_token: str) -> dict:
    ...
    with httpx.Client() as client:
        ...
        if resp.status_code in (401, 403):
            raise RuntimeError("Gemini token refresh failed. Re-authenticate with: deepalpha auth gemini")
        resp.raise_for_status()
        return resp.json()  # type: ignore[no-any-return]
...
class GeminiTokenManager:
    ...
    def _refresh(self) -> str:
        if not self._refresh_token:
            creds = self._store.load()
            if not creds:
                raise RuntimeError("No Gemini credentials stored. Run: deepalpha auth gemini")
            self._refresh_token = creds["refresh_token"]
...
def gemini_login_interactive() -> None:
    ...
    console.print()
    console.print("Use with:  [cyan]deepalpha --provider gemini-cli[/cyan]")
    console.print("       or: [cyan]deepalpha --model gemini-3-pro-preview[/cyan]")
    console.print()
