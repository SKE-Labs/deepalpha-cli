...
### Skill Location for DeepAlpha

In deepalpha CLI, skills are stored in `~/.deepalpha/<agent>/skills/` where `<agent>` is your agent configuration name (default is `agent`). For example, with the default configuration, skills live at:

```
~/.deepalpha/agent/skills/
├── skill-name-1/
│   └── SKILL.md
├── skill-name-2/
│   └── SKILL.md
...
For deepalpha CLI, use the agent's skills directory:

```bash
scripts/init_skill.py <skill-name> --path ~/.deepalpha/agent/skills
```
